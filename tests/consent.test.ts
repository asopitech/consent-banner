...
describe('consent banner', () => {
  ...
  it('reshows on version change and after reset', async () => { localStorage.setItem('asopiTechConsent', JSON.stringify({ version: 'old', choice: 'granted', updatedAt: new Date().toISOString() })); await boot(); expect(document.querySelector('[role="region"]')).toBeTruthy(); window.AsopiConsentBanner.reset(); expect(document.querySelector('[role="region"]')).toBeTruthy(); });
